...
Schema v1 可缺少 `visual_orientation`，runtime 會安全補 unknown；Schema v2 起要求完整方向欄位；目前新 Provider 請求與 plan／cache 身分使用 Schema v3，v1／v2 僅保留歷史相容。Migration 19 保存原始 EXIF、AI 結果與人工設定。新照片在本機掃描就保存 EXIF；同路徑 SHA 改變時會清除所有舊 AI、人工、裁切與本機特徵，重新寫入新 EXIF。內容未變不清除人工設定。Exact duplicate 可繼承 AI 方向結果，不能繼承人工設定。
