...
目前 Provider 請求與 plan／cache 身分使用嚴格 Schema v5（已存 v4 仍可相容讀取），`visual_orientation` 是必要欄位，不接受 v1–v3 回應或自動補欄位。已存 v4／v5 有效 semantic 結果可參與現行排名；v1–v3 保留作歷史資料，需明確重新分析才進入現行 semantic 排名。Migration 19 保存原始 EXIF、AI 結果與人工設定。新照片在本機掃描就保存 EXIF；同路徑 SHA 改變時會清除所有舊 AI、人工、裁切與本機特徵，重新寫入新 EXIF。內容未變不清除人工設定。Exact duplicate 可繼承 AI 方向結果，不能繼承人工設定。

Prefilter、local fallback 與其他未呼叫 Provider 的結果一律帶 unknown orientation，但不具方向權威性，因此不會覆蓋既有 AI 建議。Manifest 的 `photo_orientations` 由本次渲染時累積的實際畫面照片建立；Adaptive Memory 不會在渲染完成後重新選配，主圖與動態選出的第二張會依畫面順序各記錄一次。
