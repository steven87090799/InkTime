...
Prefilter、local fallback 與其他未呼叫 Provider 的 v2 結果一律帶 unknown orientation，但不具方向權威性，因此不會覆蓋既有 AI 建議。Manifest 的 `photo_orientations` 由本次渲染時累積的實際畫面照片建立；Adaptive Memory 不會在渲染完成後重新選配，主圖與動態選出的第二張會依畫面順序各記錄一次。
