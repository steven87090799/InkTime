...
目前 Provider 請求與 plan／cache 身分使用嚴格 Schema v4，`visual_orientation` 是必要欄位，不接受 v1–v3 回應或自動補欄位。舊版已儲存結果保留作歷史資料，需重新分析才進入 v4 排名。Migration 19 保存原始 EXIF、AI 結果與人工設定。新照片在本機掃描就保存 EXIF；同路徑 SHA 改變時會清除所有舊 AI、人工、裁切與本機特徵，重新寫入新 EXIF。內容未變不清除人工設定。Exact duplicate 可繼承 AI 方向結果，不能繼承人工設定。
