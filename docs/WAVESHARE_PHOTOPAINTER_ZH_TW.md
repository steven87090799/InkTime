...
InkTime 2.3.0 可用單一 compile-time Profile 切換既有 PCB 與 Waveshare
...
|---|---:|---:|---:|
| 既有 GDEY | Release | 1,212,877／1,310,720 bytes（92%） | 96,556 bytes（29%） |
| 既有 GDEP | Release | 1,212,965／1,310,720 bytes（92%） | 96,564 bytes（29%） |
| 既有 GDEY | Debug | 1,288,701／1,310,720 bytes（98%） | 96,604 bytes（29%） |
| PhotoPainter | Release | 1,161,411／3,145,728 bytes（36%） | 49,168 bytes（15%） |
| PhotoPainter | Debug | 1,253,755／3,145,728 bytes（39%） | 49,296 bytes（15%） |

既有板 Debug 僅餘 22,019 bytes app 空間，僅適合短期診斷；PhotoPainter 的雙 OTA
