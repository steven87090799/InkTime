...
裝置依序回報 `MANIFEST_RECEIVED`、`DOWNLOAD_STARTED`、`DOWNLOAD_COMPLETED`、`HASH_VERIFIED`、`DISPLAY_STARTED`、`DISPLAY_COMPLETED` 或 `DISPLAY_FAILED` 至 `POST /api/device/queue/ack`。每次 payload 必須包含 `queue_item_id`、`event` 與穩定 `idempotency_key`；不可回報其他裝置的 Item。只有 Display Completed 視為成功顯示並更新 Last Known Good。
