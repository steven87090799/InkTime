...
若 `GET /api/device/v1/offline-schedule` 的今日資料尚未完成，伺服器回傳 `404` 與 `schedule_not_ready`、`retry_after_epoch`，不要求裝置每 60 秒輪詢。韌體只接受「晚於現在且不超過合理上限」的 epoch；缺少、過期或不合法時使用持久化 bounded retry：15 分鐘、30 分鐘，之後最多 60 分鐘。成功切換 active schedule 會清除 retry state；無 active schedule 或已過期也使用同一策略，不會盲睡 24 小時。
...
- Enhanced 的 `local_next` 只是一個 cache-only 人工預覽，不會消費未來 Slot，也不會寫 terminal ACK／ACK journal；正式排程 timer wake 才擁有 Slot，並以實際 wake event epoch 回報 `DISPLAY_COMPLETED` 或 `DISPLAY_FAILED`。相同 SHA 可略過物理刷新，但不能略過正式 ACK。
