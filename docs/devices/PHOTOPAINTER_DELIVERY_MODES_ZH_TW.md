...
- `schedule_times` 排序、去重；未確認或舊裝置最多 12 個正式 Slots，只有 2.8.0 pairing／repair capability 明確宣告 `offline_schedule_max_slots=24` 且資料庫狀態為 `confirmed_24` 才可使用 24。舊資料超過 12 但無可信能力時標成 `legacy_ambiguous`、隔離並回 `DEVICE-008`；`prefetch_lead_minutes` 為 0–120。
...
- 延遲 terminal ACK 例外只允許 `DISPLAY_COMPLETED` 或 `DISPLAY_FAILED`，且 Queue Item 必須是 offline prefetched、已經 ACK/display 狀態、Release identity 與 deadline 都相符；`DISPLAY_STARTED` 不屬於 delayed terminal。韌體先把 ACK identity 寫入 crash-consistent NVS journal，Server 未以 2xx 接受前不得清除或解鎖下一張；一般 Online Queue 仍採嚴格 queue-version。
