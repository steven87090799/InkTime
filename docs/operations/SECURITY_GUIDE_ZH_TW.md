...
- 自動配對 Device Secret 為高熵長效 credential，資料庫只存 HMAC-SHA256 與版本；claim 只交付一次，完整值不進 URL／Log。既有 Legacy Device Token 仍只存 HMAC。Release、status 與 Queue manifest/file/ACK 共用同一認證入口；缺少／錯誤／撤銷 credential 回 401，失敗嘗試超限回 429 並帶 `Retry-After`。
