...
管理畫面以「發生什麼事、影響、可以怎麼處理」為主，不再要求使用者查代碼表。
照片、工作項目、執行監控、AI 追蹤、批次分析、裝置、模型設定、錯誤中心及全站通知
共用 `inktime/app/web/error_messages.py` 的繁體中文說明；瀏覽器使用同一份目錄。
目錄由可快取的同源資源提供，透過 ETag 重新驗證，不在每個頁面重複嵌入整份文字。
原始識別碼與已遮蔽訊息保留在預設收合的「技術資料」，方便維運對照日誌。

- 管理 API 的失敗回應新增 `user_error`，包含 `title`、`detail`、`action`、`message`。
  原本的 `error_code`、`message` 或 `error.code` 不變；HTTP 狀態與 `Retry-After` 不變。
- 裝置通訊及健康檢查 API 不加入畫面欄位，維持原協定；管理頁仍可解釋裝置錯誤。
- 成功 API 回應、原始模型 JSON、Token／成本、重試規則與權限不被這個顯示層修改。
- 同一識別碼依具體回報補充原因，例如圖片適合度格式錯誤、截圖禁止上傳、模型金鑰失效、
  額度不足、頻率限制或沒有相容端點。不能僅由代碼判定的原因，會明確標示尚未分類。
- 已成功工作的舊錯誤只留作歷史；已恢復事件不再顯示成仍需處理的故障。
- 新增錯誤時須同步補上說明與下一步。`tests/unit/test_error_messages.py` 檢查編號型代碼、
  主要具名錯誤家族的覆蓋率、瀏覽器／伺服器一致性、遮蔽及介面相容性。

以下代碼表供維運查詢，不是使用者處理問題的先決條件。
