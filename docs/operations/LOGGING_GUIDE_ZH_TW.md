...
stdout/stderr 仍由 Docker `json-file` 收集並輪替。應用 Log 與 HTTP access log 分開：應用 Log 可在 Web 動態改層級，access log 為部署層且預設關閉，避免健康檢查與裝置輪詢產生大量無用寫入。結構化 Log 與 AI Trace 是不同功能：主線目前依序套用至 Migration 63，其中 Migration 51 新增 AI Trace tables，Migration 52 新增 Provider 模型欄位。
...
`.env` 的 `INKTIME_LOG_LEVEL=DEBUG` 是程序啟動預設；資料庫已有 `system.log_level` 時，以保存值為準，須在「設定」同步改成 DEBUG。`INKTIME_ENVIRONMENT=development` 與開發 Compose 控制本機 HTTP 行為，和 Log 層級是不同設定。重建應沿用 `/data` 中的資料庫與 `session.key`，保留帳號、模型、加密憑證與個人設定；不要重設管理員或產生新 Session Key。
