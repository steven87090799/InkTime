...
Queue 使用單調 queue version 和每項 idempotency key。只有符合目前裝置、Queue Item、version、Release 與 event identity，且由 Server 2xx 接受的 `DISPLAY_COMPLETED` ACK 能完成項目、寫入顯示歷史並更新 Last Known Good；current/LKG 依 `displayed_at` 時間單調前進，順序相反的晚到 ACK 只能補歷史，不能倒退指標；同一時間若出現不同 Release 則拒絕。韌體 Server ACK 前保留 crash-consistent NVS journal；重複、錯誤裝置、未知項目、錯誤 key 或非權威 ACK 均拒絕／冪等處理且不能解鎖下一張。
...
Slot capability 也不可只依賴 UI：未確認／Legacy 為 12，只有 pairing／repair 明確確認才是 24；`legacy_ambiguous` 必須隔離。Canary 與 rollback 不得繞過此能力或 delivery-mode guard。
