...
## 2026-09-03 現行文件與升級設定修復

- 文件依 main `51309e2` 校正至 Migration 57、Vision v4、目前設定頁與部署契約；保留既有分類及歷史驗收日期。
- 整合完整 NAS／Web／ESP32 上線指南與 7C 韌體入口。
- 設定快照略過退休 key 並保留原始資料，修復舊資料庫無法儲存一般設定的問題。
- 恢復照片庫對既有模型歷史的顯示與搜尋，詳細頁保留原始分數；儀表板區分本機完成狀態、現行 v4 與歷史模型張數，不改寫分析資料或排名契約。
