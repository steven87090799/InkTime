...
核對日期：2026-08-31。原始碼基準：`origin/main` 的 `48d2b8d`；本文只描述這個主線版本，其他 worktree 中尚未合併的功能不列為已上線。部署中的版本請另從「診斷」核對 Git revision。
...
| SQLite Migration | 連續 `1–52` | [`migrations.py`](../../inktime/app/db/migrations.py) |
| AI Analysis Schema | 新請求 v3；保留 v1/v2 讀取相容 | [`plan.py`](../../inktime/app/domain/analysis/plan.py)、[`schema.py`](../../inktime/app/domain/analysis/schema.py) |
...
操作見[管理員指南](../guides/ADMIN_GUIDE_ZH_TW.md)、[本機選片](../guides/LOCAL_ONLY_SELECTION_ZH_TW.md)與[Activity／AI Trace](../guides/ACTIVITY_AI_TRACE_ZH_TW.md)。本主線設定頁仍有「基本／進階」篩選；未合併的全站搜尋或介面改版不屬於此基線。
...
本次文件核對沒有執行 pytest、容器建置、瀏覽器實測、付費 API、NAS 更新或刷機；不能作為這些驗收的 PASS。歷史 CI 與量測保存原日期。PhotoPainter 2026-08-22／23 的局部實板結果仍見[硬體交接](../devices/PHOTOPAINTER_REV2_TG28_HARDWARE_HANDOFF_ZH_TW.md)，不推廣為目前全部功能已驗收。
