...
def test_orientation_priority_and_thresholds():
    assert (
        resolve_effective_orientation(
            exif_orientation=6, manual_rotation_cw=None, ai_rotation_cw=90, ai_confidence=0.95
        ).source
        == "ai"
    )
    assert (
        resolve_effective_orientation(
            exif_orientation=6, manual_rotation_cw=None, ai_rotation_cw=180, ai_confidence=0.95
        ).source
        == "exif_normalized"
    )
    assert (
        resolve_effective_orientation(
            exif_orientation=1, manual_rotation_cw=270, ai_rotation_cw=90, ai_confidence=1
        ).as_dict()["source"]
        == "manual"
    )
    assert (
