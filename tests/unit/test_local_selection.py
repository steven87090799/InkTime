    ...
    with app.extensions["inktime_database"].session() as connection:
        connection.execute(
            ...
            (
                ...
                1600,
                score,
                f"2020-{month_day}T10:00:00+00:00",
                f"2020-{month_day}",
                month_day,
                "2020-01-01",
                "2020-01-01",
...
def test_history_fallback_and_leap_day_are_explicit_and_bounded(app, tmp_path):
    ...
    with app.extensions["inktime_database"].session() as connection:
        trace = connection.execute(
            "SELECT context_snapshot_json FROM selection_decision_traces WHERE trace_id=?",
            (leap["decision_trace_id"],),
        ).fetchone()
...
def test_pair_secondary_stays_inside_effective_fallback_pool(app, tmp_path):
    ...
    settings.update("render.history_today_fallback", "nearby_only", changed_by="test", source_ip="test")
    policy = LocalSelectionPolicy(
        app.extensions["inktime_database"], settings, app.extensions["inktime_resilience_repository"]
    )
    result = policy.select(target=date(2026, 7, 28), orientation="portrait", quantity=2, layout="photo_pair")
    ...
    assert set(components["pair_score_components"]) >= {
        "primary_local_display_score",
        "secondary_local_display_score",
        "orientation_compatibility",
        "capture_time_proximity",
        "known_location_match",
        "location_data_available",
        "dual_low_priority_penalty",
        "dual_high_epaper_risk_penalty",
        "dual_recent_display_penalty",
    }
...
def test_pair_score_uses_only_offline_resolved_city(app, tmp_path):
    ...
    with app.extensions["inktime_database"].session() as connection:
        connection.execute(
            "UPDATE photos SET gps_lat=25.05306,gps_lon=121.52639 WHERE id IN ('taipei-a','taipei-b')"
        )
    policy = LocalSelectionPolicy(
        app.extensions["inktime_database"],
        app.extensions["inktime_settings_repository"],
        app.extensions["inktime_resilience_repository"],
        app.extensions["inktime_location_resolver"],
    )
    result = policy.select(target=date(2026, 7, 28), orientation="portrait", quantity=2, layout="photo_pair")
