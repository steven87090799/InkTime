...
struct SensorSleepBus {
  bool asleep = false;
  bool wakeFails = false;
  bool sleepAckLost = false;
  uint32_t elapsedMs = 0U;
  std::vector<uint16_t> commands;

  bool writeCommand(uint8_t address, uint16_t command, bool replaySafe) {
    assert(address == 0x70U);  // No writes to the shared PMIC/RTC/codecs.
    assert(!replaySafe);      // An accepted sleep with lost ACK is not replayed.
    commands.push_back(command);
    if (command == 0x3517U) {
      if (wakeFails) return false;
      asleep = false;
      return true;
    }
    assert(command == 0xB098U);  // ID reads/conversions are forbidden here.
    assert(!asleep && elapsedMs >= 1U);
    asleep = true;
    return !sleepAckLost;
  }
};

static void testUnusedSensorSleep() {
  using namespace photopainter_sensor;
  for (bool alreadyAsleep : {false, true}) {
    SensorSleepBus bus;
    bus.asleep = alreadyAsleep;
    assert(disableMeasurements(bus, [&](uint32_t ms) { bus.elapsedMs += ms; })
           == SleepResult::CommandAccepted);
    assert(bus.asleep);
    assert(bus.commands == std::vector<uint16_t>({0x3517U, 0xB098U}));
  }
  SensorSleepBus absent;
  absent.wakeFails = true;
  assert(disableMeasurements(absent, [&](uint32_t ms) { absent.elapsedMs += ms; })
         == SleepResult::WakeFailed);
  assert(absent.commands.size() == 1U && absent.elapsedMs == 0U);

  SensorSleepBus uncertain;
  uncertain.sleepAckLost = true;
  assert(disableMeasurements(uncertain, [&](uint32_t ms) { uncertain.elapsedMs += ms; })
         == SleepResult::SleepUnconfirmed);
  assert(uncertain.asleep && uncertain.commands.size() == 2U);
}

int main() {
  testUnusedSensorSleep();
