...
def _safe_join(base: Path, rel: str) -> Path:
    """相容入口統一使用新版 Path containment 驗證。"""
...
def esp_photo(key: str, idx: int):
    if not ENABLE_LEGACY_DEVICE_API or not DOWNLOAD_KEY or key != DOWNLOAD_KEY:
        abort(404)
...
if __name__ == "__main__":
    ...
    print(f"[InkTime] 舊版裝置 API: {'已啟用（不安全）' if ENABLE_LEGACY_DEVICE_API else '已停用'}")
