...
_register("BATCH-FILE-LIMIT BATCH-OUTPUT-SIZE-001", "批次結果超過安全大小限制", "下載或匯入的結果檔超過允許的總大小，系統已停止處理。", "保留遠端批次與用量紀錄，拆分結果或調整受控上限後再匯入，勿重送照片。")
_register("BATCH-LIFECYCLE-REQUIRED", "批次請求必須使用批次管理流程", "目前呼叫方式無法保存提交、輪詢與計費狀態，因此未送出批次。", "從批次分析頁建立工作，保留完整生命週期與計費紀錄。")
_register("JOB-LOCAL-FAILED", "本機背景處理失敗", "隔離的背景程序未能完成處理，詳細原因已保存於日誌。", "查看背景工作日誌並修正具體原因，再確認是否需要重跑。")
_register("JOB-LOCAL-TIMEOUT", "本機背景處理逾時", "工作超過執行期限，隔離程序已被停止。", "查看資料大小與主機資源，確認輸出狀態後再處理失敗項目。")
_register("PROVIDER-022", "模型能力與請求不相容", "模型不支援所需的圖片輸入或推理設定，因此請求尚未送出。", "選擇支援圖片的模型，並使用該模型允許的推理設定。")
_register("SESSION-002", "登入金鑰無法安全載入", "登入金鑰檔案狀態不符合安全要求，系統拒絕使用。", "由管理員檢查持久化資料目錄的金鑰檔案類型、權限及掛載設定。")
_register("SESSION-003", "主密鑰與資料庫不符", "目前金鑰與資料庫記錄的指紋不同，系統已停止啟動以保護裝置配對與儲存的憑證。", "還原與資料庫配套的 session.key 或 INKTIME_SECRET_KEY；只有確認接受重新配對及重建憑證時才由管理員輪替密鑰。")
_register("JOB-005", "背景工作主迴圈遇到錯誤", "Worker 已記錄異常，會在短暫等待後繼續處理。", "查看工作狀態與背景日誌；若同一錯誤持續出現，先修正該工作的資料或設定。")
_register("JOB-SHUTDOWN-CANCELLED", "本機工作因服務停止而中斷", "隔離的本機程序已停止，可重跑的項目將依重試規則重新排入。", "服務恢復後查看原工作進度；不需要另建相同工作，此狀態不授權重送未知結果的付費請求。")
_register("VLM-INCOMPLETE", "模型回應未完整完成", "模型拒答、截斷或過濾了回應，系統已保留收到的內容與用量。", "核對回應與用量後處理，不要直接重送圖片或付費修復。")
_register("VLM-UNRECONCILED", "先前模型請求尚待對帳", "系統無法確認先前請求的結果，為避免重複費用已阻止再次送圖。", "先核對模型服務端請求與用量，完成對帳或記錄明確的重送批准。")
...
_register("AI-IMAGE-001", "送入模型的圖片尚未完成格式轉換", "本次圖片不是可供模型讀取的標準預覽格式，系統已在送出請求前停止。", "先確認照片能產生預覽圖；由管理員檢查圖片轉換流程，修復後再重跑失敗項目。")
_register("IMG-UNSUPPORTED", "目前不支援這種照片格式", "檔案格式不在可處理範圍內，包含目前尚未支援的 iPhone ProRAW DNG。", "保留原始檔，另外匯出 JPEG 或其他支援格式後重新掃描；不要只更改副檔名。")
_register("IMG-MISSING", "找不到原始照片", "原始檔可能已移動、刪除，或照片來源尚未掛載。", "確認照片來源掛載與原始檔位置，恢復來源後重新掃描。")
_register("IMG-IO", "照片讀取或圖片處理目錄無法使用", "原始檔存取、資料目錄或解碼鎖的安全檢查未通過，也可能是磁碟讀寫失敗。", "請管理員檢查照片與資料目錄的掛載、權限、可用空間及日誌；不要關閉安全檢查。")
_register("IMG-FILE-LIMIT", "原始照片檔案超過大小上限", "檔案大小超過圖片處理的安全限制，本次未繼續解碼。", "保留原始檔，另外匯出較小的照片副本後重新掃描；不要停用安全上限。")
_register("IMG-HEIF-UNAVAILABLE", "目前無法解碼 HEIC／HEIF 照片", "服務缺少可用的 HEIF／HEVC 解碼器，不能產生這類照片的預覽圖。", "請管理員檢查圖片能力診斷與服務安裝；修復解碼器後再試，重複提交不會補上解碼器。")
_register("IMG-DECODE-BUSY", "圖片解碼仍在忙碌中", "等待共用圖片解碼名額逾時，這不代表原始照片損壞。", "等待目前圖片處理完成後再試；若持續發生，請管理員檢查背景服務與解碼鎖。")
_register("IMG-CORRUPT", "圖片內容無法解碼", "解碼器無法讀取圖片內容，檔案可能不完整、損壞或與格式不符。", "確認原始檔可在其他圖片工具開啟；保留原檔，必要時重新匯出照片副本後掃描。")
