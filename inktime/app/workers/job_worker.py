...
class BoundedJobWorker:
    ...
    def __init__(self, repository: JobRepository, processor: Processor, *, concurrency: int = 2, queue_multiplier: int = 2, max_attempts: int = 3) -> None:
    ...
    def run_job(self, job_id: str) -> None:
        ...
        with ThreadPoolExecutor(max_workers=self.concurrency, thread_name_prefix="inktime") as executor:
            while not self.stop_event.is_set():
                ...
                if job is None or job["status"] in {"cancelled", "completed", "completed_with_errors", "failed", "paused", "budget_exceeded"}:
                    break
                ...
                for future in done:
                    ...
                    try:
                        ...
                        code = getattr(exc, "code", "JOB-003")
                        self.repository.fail_item(job_id, item_id, str(code), str(exc), max_attempts=self.max_attempts)
                    else:
            ...
            for future in list(futures):
                ...
                try:
                    ...
                    self.repository.fail_item(job_id, item_id, getattr(exc, "code", "JOB-003"), str(exc), max_attempts=self.max_attempts)
                else:
                    self.repository.complete_item(job_id, completed_id, result, cost)
            job = self.repository.get(job_id)
