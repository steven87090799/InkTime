...
class ObservabilityService:
        ...
        with self.database.session() as c:
            c.execute(
                ...
                (
                    ...
                    component[:80],
                    event[:80],
                    redact_text(message)[:1000],
                    clean.get("job_id"),
                    clean.get("photo_id"),
                    clean.get("device_id"),
                    clean.get("stage"),
                    clean.get("progress_done"),
                    clean.get("progress_total"),
                    clean.get("error_code"),
                    clean.get("trace_id"),
    ...
    def _disable_expired_debug(self, now: datetime) -> None:
        ...
        with self.database.session() as c:
            row = c.execute(
                "SELECT updated_at FROM settings WHERE key='observability.debug_enabled'"
            ).fetchone()
            changed = self._parse(row[0]) if row else None
            if (
                self.settings.get("observability.debug_enabled", False)
                and changed
                and now - changed >= expiry
            ):
                disabled = True
        if disabled:
    ...
    def _check_jobs(self, now: datetime) -> None:
        cutoff = (
            now - timedelta(minutes=int(self.settings.get("observability.stuck_job_minutes", 5)))
        ).isoformat()
        with self.database.session() as c:
            stale = c.execute(
                "SELECT id,status,heartbeat_at FROM jobs WHERE status IN ('running','retrying') AND heartbeat_at<?",
                (cutoff,),
            ).fetchall()
        stale_ids = set()
        for job in stale:
            ...
            self.alert(
                "worker",
                "JOB-HEARTBEAT-STALE",
                f"工作 {job['id']} 卡在 {job['status']}",
                job_id=job["id"],
                subject=str(job["id"]),
                details={
                    "last_event_at": job["heartbeat_at"],
                    "waiting_seconds": int((now - (self._parse(job["heartbeat_at"]) or now)).total_seconds()),
                    "retryable": True,
                    "recommended_page": "/jobs/" + str(job["id"]),
                },
            )
        with self.database.session() as c:
            unresolved = c.execute(
                "SELECT job_id FROM job_errors WHERE error_code='JOB-HEARTBEAT-STALE' AND resolved_at IS NULL"
            ).fetchall()
        for row in unresolved:
            if str(row["job_id"] or "") not in stale_ids:
                self.recover(
                    "worker", "JOB-HEARTBEAT-STALE", "工作 heartbeat 已恢復", subject=str(row["job_id"] or "")
                )
    ...
    def _check_providers(self, now: datetime) -> None:
        ...
        with self.database.session() as c:
            rows = c.execute(
                "SELECT provider,status,error_code,completed_at,started_at FROM api_usage WHERE COALESCE(completed_at,started_at)>=? ORDER BY id DESC LIMIT 200",
                (cutoff,),
            ).fetchall()
        ...
        for provider, events in by_provider.items():
            failures = [
                row for row in events if str(row["status"]).lower() not in {"ok", "success", "completed"}
            ]
            if not failures:
                continue
            ...
            code = (
                "AI-PROVIDER-TIMEOUT"
                if "timeout" in codes
                else "AI-PROVIDER-RATE-LIMIT"
                if ("429" in codes or "rate" in codes)
            ...
            if consecutive >= 3:
                ...
                self.alert(
                    "provider",
                    code,
                    f"Provider {provider} 在 15 分鐘內連續失敗 {consecutive} 次",
                    severity=severity,
                    subject=provider,
                    details={
                        "waiting_seconds": int(self._PROVIDER_WINDOW.total_seconds()),
                        "retryable": True,
                        "recommended_page": "/providers",
                    },
                )
        ...
        with self.database.session() as c:
            pending = int(
                c.execute(
                    "SELECT COUNT(*) FROM job_items WHERE status='pending' AND available_at<?",
        ...
        if pending and active:
            self.alert(
                "provider",
                "AI-PROVIDER-COOLDOWN-STUCK",
                "工作 Queue 已等待 Provider 冷卻超過 15 分鐘",
                severity="ERROR",
                details={"waiting_seconds": 900, "retryable": True, "recommended_page": "/jobs"},
            )
        else:
            self.recover("provider", "AI-PROVIDER-COOLDOWN-STUCK", "Provider 冷卻等待已結束")
        with self.database.session() as c:
            unresolved = c.execute(
                "SELECT fingerprint FROM job_errors WHERE component='provider' AND error_code LIKE 'AI-PROVIDER-%' AND resolved_at IS NULL"
            ).fetchall()
        for row in unresolved:
            for provider, code in list(active):
                ...
                for provider in by_provider:
                    for code in ("AI-PROVIDER-TIMEOUT", "AI-PROVIDER-RATE-LIMIT", "AI-PROVIDER-UNAVAILABLE"):
                        if (
                            hashlib.sha256(f"provider:{code}:{provider}".encode()).hexdigest()
                            == row["fingerprint"]
                        ):
                            self.recover(
                                "provider", code, f"Provider {provider} 已恢復成功請求", subject=provider
                            )
    ...
    def _check_releases(self, now: datetime) -> None:
        ...
        with self.database.session() as c:
            ...
            known_ids = [
                str(row[0])
                for row in c.execute("SELECT id FROM releases ORDER BY created_at DESC LIMIT 1000").fetchall()
            ]
        active: set[tuple[str, str]] = set()
        for row in rows:
            ...
            if status == "staged" and str(row["created_at"]) < staged_cutoff:
                ...
                self.alert(
                    "release",
                    "RELEASE-STUCK",
                    f"Release {release_id} staged 超過 20 分鐘",
                    severity="ERROR",
                    subject=release_id,
                    details={
                        "last_event_at": row["created_at"],
                        "retryable": True,
                        "recommended_page": "/errors",
                    },
                )
            if status == "staged_failed" or str(row["reconciliation_status"]) == "payload_missing":
                ...
                self.alert(
                    ...
                    f"Release {release_id} 驗證或 Payload 檢查失敗",
                    severity="ERROR",
                    subject=release_id,
            ...
            if str(row["reconciliation_status"]) not in {"ok", "applied", "skipped"}:
                ...
                self.alert(
                    "release",
                    "RELEASE-POINTER-DRIFT",
                    f"Release {release_id} 的 DB 與 latest pointer 狀態不一致",
                    severity="CRITICAL",
                    subject=release_id,
                    details={"retryable": True, "recommended_page": "/diagnostics"},
                )
            if "rollback" in str(row["failure_reason"] or "").lower():
                ...
                self.alert(
                    ...
                    f"Release {release_id} 補償或 rollback 失敗",
                    severity="CRITICAL",
                    subject=release_id,
                    details={"retryable": False, "recommended_page": "/errors"},
                )
        with self.database.session() as c:
            errors = c.execute(
                "SELECT fingerprint FROM job_errors WHERE component='release' AND resolved_at IS NULL"
            ).fetchall()
        for row in errors:
            for release_id in known_ids:
                for code in (
                    ...
                    if (release_id, code) not in active and hashlib.sha256(
                        f"release:{code}:{release_id}".encode()
                    ).hexdigest() == row["fingerprint"]:
                        self.recover("release", code, "Release 已發布、修復或安全回滾", subject=release_id)

    def _check_schedules(self, now: datetime) -> None:
        with self.database.session() as c:
            tasks = c.execute(
                "SELECT key,kind,next_run,last_success,last_failure,error_status,timeout_seconds,enabled FROM scheduled_tasks WHERE enabled=1 AND next_run IS NOT NULL ORDER BY next_run LIMIT 50"
            ).fetchall()
        for task in tasks:
            due = self._parse(task["next_run"])
            if not due or now <= due + timedelta(seconds=max(300, int(task["timeout_seconds"]))):
                for code in (
                    "SCHEDULE-PREPARE-OVERDUE",
                    "SCHEDULE-RELEASE-OVERDUE",
                    "SCHEDULE-REPEATED-SKIP",
                ):
                    self.recover(
                        "scheduler", code, f"排程 {task['key']} 已回到正常執行區間", subject=str(task["key"])
                    )
                continue
            code = (
                "SCHEDULE-PREPARE-OVERDUE"
                if str(task["key"]) == "display_prepare"
                else "SCHEDULE-RELEASE-OVERDUE"
            )
            ...
            self.alert(
                "scheduler",
                code,
                f"排程 {task['key']} 超過允許延遲仍未完成",
                severity=severity,
                subject=str(task["key"]),
                details={
                    "last_success_at": task["last_success"],
                    "last_event_at": task["last_failure"] or task["next_run"],
                    "waiting_seconds": int((now - due).total_seconds()),
                    "retryable": True,
                    "recommended_page": "/schedules",
                },
            )
            for alternate in {
                "SCHEDULE-PREPARE-OVERDUE",
                ...
                self.recover(
                    "scheduler", alternate, f"排程 {task['key']} 的先前狀態已恢復", subject=str(task["key"])
                )
    ...
    def _check_devices(self, now: datetime) -> None:
        ...
        for row in rows:
            ...
            if not downloaded or (assigned and downloaded < assigned):
                self.alert(
                    "device",
                    "DEVICE-DOWNLOAD-OVERDUE",
                    f"裝置 {device} 已指派 Release 但尚未下載",
                    severity="WARNING",
                    subject=f"{device}:{release}",
                    details={
                        "device_id": device,
                        "waiting_seconds": int((now - (assigned or now)).total_seconds()),
                        "retryable": True,
                        "recommended_page": "/devices",
                    },
                )
            else:
                self.recover(
            ...
            if downloaded and not verified and now - downloaded >= timedelta(hours=2):
                self.alert(
                    "device",
                    "DEVICE-VERIFY-OVERDUE",
                    f"裝置 {device} 已下載 Release 但尚未回報 SHA 驗證",
                    severity="WARNING",
                    subject=f"{device}:{release}",
                    details={
                        "device_id": device,
                        "waiting_seconds": int((now - downloaded).total_seconds()),
                        "retryable": True,
                        "recommended_page": "/devices",
                    },
                )
            ...
            if downloaded and verified and not displayed and now - downloaded >= timedelta(hours=2):
                self.alert(
                    "device",
                    "DEVICE-ACK-OVERDUE",
                    f"裝置 {device} 已驗證 Release 但尚未回報 DISPLAY_COMPLETED",
                    severity="WARNING",
                    subject=f"{device}:{release}",
                    details={
                        "device_id": device,
                ...
                self.recover(
                    "device",
                    "DEVICE-ACK-OVERDUE",
                    "裝置已回報 DISPLAY_COMPLETED",
                    subject=f"{device}:{release}",
                )
            if int(row["download_failure_count"] or 0) >= 3:
                self.alert(
                    "device",
                    "DEVICE-DISPLAY-FAILED",
                    f"裝置 {device} 連續下載或顯示失敗",
                    severity="ERROR",
                    subject=device,
                    details={"device_id": device, "retryable": True, "recommended_page": "/devices"},
                )
            elif downloaded:
                self.recover("device", "DEVICE-DISPLAY-FAILED", "裝置下載已恢復", subject=device)
            if int(row["acked_config_version"] or 0) < int(row["config_version"] or 0) and self._device_due(
                str(row["assigned_at"]), str(row["schedule"]), str(row["timezone"]), now
            ):
                self.alert(
                    "device",
                    "DEVICE-CONFIG-ACK-OVERDUE",
                    f"裝置 {device} 尚未確認目前設定",
                    severity="WARNING",
                    subject=device,
                    details={"device_id": device, "retryable": True, "recommended_page": "/devices"},
                )
    ...
    def alert(self, component, code, message, *, job_id=None, severity="WARNING", subject="", details=None):
        ...
        with self.database.session() as c:
            found = c.execute(
                "SELECT id,last_seen_at FROM job_errors WHERE fingerprint=? AND resolved_at IS NULL", (fp,)
            ).fetchone()
            if found:
                ...
                if self._now() - seen >= self._ALERT_INTERVAL:
                    c.execute(
                        ...
                        (now, redact_text(message)[:1000], found["id"]),
                    )
                    recorded = True
            else:
                c.execute(
                    "INSERT INTO job_errors(job_id,component,error_code,fingerprint,severity,message,first_seen_at,last_seen_at) VALUES (?,?,?,?,?,?,?,?)",
                    (job_id, component, code, fp, severity.lower(), redact_text(message)[:1000], now, now),
                )
                recorded = True
        if recorded:
            self.record(
                severity, component, "alert", message, job_id=job_id, error_code=code, **clean_details
            )
    ...
    def recover(self, component, code, message, *, subject=""):
        ...
        with self.database.session() as c:
            row = c.execute(
                "SELECT id FROM job_errors WHERE fingerprint=? AND resolved_at IS NULL", (fp,)
            ).fetchone()
            if row:
                c.execute(
    ...
    def cleanup(self, now: datetime | None = None):
        ...
        cutoff, debug_cutoff = (
            ...
            (
                now - timedelta(hours=int(self.settings.get("observability.debug_retention_hours", 24)))
            ).isoformat(),
        )
        ...
        with self.database.session() as c:
            ...
            if last and now - last < timedelta(hours=1):
                return
            c.execute(
                "DELETE FROM activity_events WHERE id IN (SELECT id FROM activity_events WHERE severity='DEBUG' AND created_at<? ORDER BY id LIMIT ?)",
                (debug_cutoff, self._BATCH_SIZE),
            )
            c.execute(
                "DELETE FROM activity_events WHERE id IN (SELECT id FROM activity_events WHERE severity='INFO' AND created_at<? ORDER BY id LIMIT ?)",
                (cutoff, self._BATCH_SIZE),
            )
            c.execute(
                "DELETE FROM activity_events WHERE id IN (SELECT a.id FROM activity_events a WHERE a.severity IN ('WARNING','ERROR') AND a.created_at<? AND NOT EXISTS (SELECT 1 FROM job_errors e WHERE e.error_code=a.error_code AND e.resolved_at IS NULL) ORDER BY a.id LIMIT ?)",
                (cutoff, self._BATCH_SIZE),
            )
            ...
            if count > max_rows:
                ...
                deleted = c.execute(
                    "DELETE FROM activity_events WHERE id IN (SELECT id FROM activity_events WHERE severity='DEBUG' ORDER BY id LIMIT ?)",
                    (budget,),
                ).rowcount
                if deleted < budget:
                    c.execute(
                        "DELETE FROM activity_events WHERE id IN (SELECT id FROM activity_events WHERE severity='INFO' ORDER BY id LIMIT ?)",
                        (budget - deleted,),
                    )
                remaining = int(c.execute("SELECT COUNT(*) FROM activity_events").fetchone()[0])
                capacity_blocked = remaining > max_rows
            c.execute(
                "INSERT INTO observability_state(key,value_json,updated_at) VALUES ('cleanup','{}',?) ON CONFLICT(key) DO UPDATE SET updated_at=excluded.updated_at",
                (now.isoformat(),),
            )
        if capacity_blocked:
            self.alert(
                "observability",
                "ACTIVITY-CAPACITY-PROTECTED",
                "Activity 已達上限，但未解決的重要事件受到保留保護",
                details={"retryable": False, "recommended_page": "/activity"},
            )
