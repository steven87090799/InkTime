...
class DiagnosticsService:
    ...
    def snapshot(self) -> dict:
        ...
        with self.database.session() as connection:
            ...
            workers = connection.execute("SELECT COUNT(*) FROM jobs WHERE status IN ('running','retrying','pausing') AND heartbeat_at IS NOT NULL").fetchone()[0]
        revision = os.environ.get("INKTIME_GIT_REVISION", "unknown")
        if revision == "unknown":
            try:
                revision = subprocess.run(["git", "rev-parse", "--short", "HEAD"], capture_output=True, text=True, timeout=2, check=True).stdout.strip()
            except Exception:
                pass
        return {
            ...
            "database": {"bytes": self.database.path.stat().st_size if self.database.path.exists() else 0, "wal_bytes": wal.stat().st_size if wal.exists() else 0, "integrity": self.database.integrity_check()},
    ...
    def bundle(self) -> BytesIO:
        ...
        with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as bundle:
            ...
            bundle.writestr("README.txt", "此診斷包不包含 API Key、Token、密碼、Session、Cookie、精確私人路徑、GPS 或原始照片。\n")
        output.seek(0)
        return output
