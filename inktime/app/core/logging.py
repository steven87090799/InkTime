...
STANDARD_FIELDS = {
    "timestamp": "", "level": "", "component": "", "event": "", "error_code": "",
    "message": "", "job_id": "", "photo_id": "", "provider": "", "model": "",
    "duration_ms": 0, "retry_count": 0, "details": {},
}
...
def configure_logging(format_name: str | None = None, level: str | None = None) -> None:
    ...
    selected = (format_name or os.environ.get("INKTIME_LOG_FORMAT", "human")).lower()
    handler.setFormatter(JsonFormatter() if selected == "json" else HumanFormatter())
    root = logging.getLogger()
    root.handlers.clear()
    root.addHandler(handler)
    root.setLevel((level or os.environ.get("INKTIME_LOG_LEVEL", "INFO")).upper())
