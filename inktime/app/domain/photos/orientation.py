...
def resolve_effective_orientation(
    *,
    exif_orientation: int | None,
    manual_rotation_cw: int | None,
    ai_rotation_cw: int | None,
    ai_confidence: float | None,
    ai_ambiguous: bool = False,
    ...
    if (
        ai_rotation_cw in ROTATIONS
        and not ai_ambiguous
        and confidence is not None
        and confidence >= threshold
    ):
        return EffectiveOrientation(ai_rotation_cw, "ai", confidence, exif_normalized)
    return EffectiveOrientation(
        0, "exif_normalized" if exif_normalized else "none", confidence, exif_normalized
    )
