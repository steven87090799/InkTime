...
SETTING_DEFINITIONS: dict[str, dict[str, Any]] = {
    ...
    "analysis.e6_prefilter_enabled": {
        ...
        "default": True,
        "type": "boolean",
        "description": "舊版設定；E6 保留作為顯示參考，不參與 AI 排名或排除",
        "risk": "保留供既有設定與歷史版本查閱；目前不影響自動選片",
        "restart": False,
    },
    "analysis.e6_min_score": {
        ...
        "default": 25,
        "type": "number",
        "description": "舊版門檻；目前不依 E6 分數自動排除照片",
        "risk": "保留供既有設定與歷史版本查閱；目前不影響自動選片",
        "min": 0,
        "max": 100,
        "restart": False,
    ...
    "offline.server_prefetch_margin_minutes": {
        ...
        "default": 15,
        "type": "integer",
        "description": "離線排程在裝置 Prefetch 前預留的伺服器準備緩衝（分鐘）",
        "risk": "過短可能來不及完成渲染；過長會增加暫存內容保留時間",
        "min": 0,
        "max": 60,
        "restart": False,
    },
    ...
    "model.repair_model": {
        ...
        "default": "gpt-4o-mini",
        "type": "string",
        "description": "只接收文字的 JSON 修復模型；不會收到圖片",
        "risk": "修復模型不可取代主 Vision 分析，也不可重新上傳照片",
        "restart": False,
    },
    "model.high_model": {
        "category": "模型設定",
    ...
    "budget.unknown_request_reserve": {
        "category": "成本設定",
        "default": 0.25,
        "type": "number",
        "description": "每筆尚未完成成本定價的請求預留金額（美元）",
        "risk": "只對有可計費證據的 unknown 請求計入；同照片或同工作仍會阻擋重試",
        "min": 0.01,
        "max": 100,
        "restart": False,
    },
    ...
    "budget.repair_max_tokens": {
        ...
        "default": 1200,
        "type": "integer",
        "description": "文字 JSON 修復請求的硬上限",
        "risk": "修復不可重新上傳圖片；實際值仍受 budget.max_tokens 夾制",
        "min": 256,
        "max": 1200,
        "restart": False,
    },
    "render.memory_threshold": {
        ...
        "default": 70,
        "type": "number",
        "description": "舊版設定；AI-first 直接依完整模型選片分排序",
        "risk": "保留供既有設定與歷史版本查閱；目前不影響自動選片",
        "min": 0,
        "max": 100,
        "restart": False,
    ...
    "render.e6_weight": {
        ...
        "default": 20,
        "type": "number",
        "description": "舊版設定；AI-first 排名不再混合 E6 分數",
        "risk": "保留供既有設定與歷史版本查閱；目前不影響自動選片",
        "min": 0,
        "max": 60,
        "restart": False,
    ...
    def ensure_defaults(self) -> None:
        ...
        with self.database.session() as connection:
            ...
            existing_rows = connection.execute(
                "SELECT key,value_json,updated_by FROM settings"
            ).fetchall()
            existing = {
            ...
            normalized_caption_values = normalize_caption_controls(
                {
                    key.removeprefix("analysis."): stored_caption_values[key]
            ...
            connection.executemany(
                ...
                [
                    (
                        json.dumps(normalized_caption_values[key.removeprefix("analysis.")]),
                        now,
                        key,
                    )
                    for key in caption_keys
                    if stored_caption_values[key]
                    != normalized_caption_values[key.removeprefix("analysis.")]
                ],
            )
            connection.execute(
                "UPDATE settings SET value_json='1024',updated_at=? "
                "WHERE key='analysis.image_max_side' AND value_json='512'",
                (now,),
            )
